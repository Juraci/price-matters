...
- **Env var**: `VITE_BRAPI_API_KEY` (Vite `VITE_` prefix is required for browser access). The key is inlined into the built bundle; acceptable for this local-only tool. `.env.example` is committed; `.env` / `.env.*` are gitignored. `playwright.config.ts` injects `VITE_BRAPI_API_KEY=test-key` for E2E so tests don't need a real key.
